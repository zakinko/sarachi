...
BitLocker は FVEK（実際にディスクを暗号化する鍵）を VMK が包み、VMK を複数の
protector が包む。protector を足し引きしてもディスクを暗号化し直さずに済む
構造で、LUKS の「マスター鍵＋keyslot」と同じ形をしている。

| protector | 位置 |
|---|---|
| TPM | 既定。無人で起動する道 |
| 回復パスワード（48 桁） | 非常口。**これが逃がされる物** |
| Network Unlock | 有線網の WDS サーバから開く |

逃がし先は今は **Entra ID の device object**。組織が管理する Windows 機の
回復鍵は、Microsoft のクラウドにある device object に預けられている。つまり
**Windows の「control plane」は Entra ID そのもの**。

**我々が TPM を正に据えられないのは、標的に Raspberry Pi を含めたため。**
そこだけが BitLocker をそのまま写せない点で、裏を返せば control plane を正に
置く判断は Pi を入れた時点でほぼ決まっていた。

**NetBSD では好みではなく必然になる。** cgd は keyslot を持たないので、鍵を
手元に置くと消去がただのファイル削除になり、SD や SSD で成立しない。

我々も Entra ID の device object へ逃がせば、**管理者の手順が Windows と
同じになる**。`libhimmelblau` が使える以上、他の Linux 用 MDM に無い筋になる。

#### 順序（実装で直した誤り）

**預けてから容器を作る。** 逆だと、預け先に届かなかったときに「暗号化されて
いるが誰も開けられない塊」が残る。`format` は中身を失わせるので、そこを
越えてから失敗してはいけない。**消去の前に救済が壊れる形**で、しかも壊れた
ことは次に開こうとするまで分からない。順序だけで防げるなら順序で防ぐ。

#### control plane（`crates/control-plane`）

```text
POST /v1/escrow            預かる
GET  /v1/key/<device_id>   渡す。起動時に取りに来る台のため
POST /v1/revoke/<id>       失効させる。**これが消去の本体になる台がある**
```

枠組みを入れず `std` だけで書いた。**鍵を預かる物の中身が読み切れなくなるのが
一番まずい**ため。置き場も台ごとに一つのファイル。台数で困るようになったら
その時に変える。

決めたこと。

- **既に在る鍵は上書きしない。** 同じ名前で二度来たときに二台目が一台目の鍵を
  消すと、救えなくなる方向にしか転ばない。作り直すなら先に失効させる
- **`revoke` は本当に消す。** 印を付けて隠すのではない。NetBSD ではこれが
  消去の本体になる
- **台の名前は外から来る**ので、置き場の外を指せないように狭めた
- **token の比較は時間で漏らさない形**にした。預かっている物を考えれば
  惜しむ所ではない
- **TLS はここで終端しない。** 前段に任せる。回復環境は https でなければ鍵を
  やりとりしないので、前段が無ければ繋がらないだけで、黙って平文で流れる形に
  はならない

socket を通す試験を置いた。本文の読み書きが単体で合っていても、回復環境と
同じ言葉を話していなければ意味が無く、食い違えば導入の終盤——鍵を預ける所——
で初めて分かることになる。

#### まだ無いもの

**台ごとに token を分けていない。** 今は一つなので、**一台分が漏れれば全台の
鍵が取れる。** 分けるには台の登録という別の仕組みが要る。運用に載せる前に
必ず直すこと。`main.rs` の頭にも書いてある。

**TPM は実装していない。** 補助であって正ではないので、無くても設計は成り
立つ。暗号層と同じく、黙って別の手に落ちずに断る。
...
cgd を実装する前に `cgdconfig(8)` を読んで分かった。**NetBSD は他の三つと
消去の成り立ちが違う。**

LUKS の `luksErase` と geli の `kill` は、**暗号化されたディスク上の鍵材料**を
壊す。ディスクそのものが鍵の入れ物を持っていて、そこを潰せば中身は戻らない。

cgd にはそれが無い。鍵は params ファイルの側にある。

| key_method | 鍵の在処 |
|---|---|
| `storedkey` | **params ファイルの中** |
| `pkcs5_pbkdf2` / `argon2id` | params の salt ＋ 人が打つパスフレーズから導出 |
| `randomkey` / `urandomkey` | 毎回 `/dev/random`。swap 用で、再起動で消える |
| `shell_cmd` | **コマンドの stdout から読む** |

素直に `storedkey` を使うと、消去は params ファイルを消すことになる。
**それは crypto-erase を選んだ理由そのものに反する。** SD と SSD では
ウェアレベリングのせいで上書きも削除も消去として成立せず、だから鍵を壊す
方式にしたのに、その鍵が普通のファイルとして置かれていては元の木阿弥になる。
しかも params ファイルは root の外（ESP など暗号化されていない所）に置く
必要がある。

#### `shell_cmd` が道になる

鍵を**手元に一切置かない**。起動のたびに control plane から取る。

    keygen shell_cmd "sarachi-key-fetch --device-id …";

こうすると、

- **消す物が手元に無い。** 消えたかどうかを flash の挙動に賭けなくて済む
- 消去は control plane で鍵を失効させることになる。中央で、確実に、即座に
- 台を持ち去られても、網の向こうが許さなければ開かない

代わりに諦めるものがある。

- **起動のたびに網が要る。** control plane が落ちれば NetBSD 機は上がらない
- 消去の保証が control plane の正直さに乗る。鍵を消したと言って消していなければ、それまで
- 非常口（パスフレーズ）を足すと、その分だけ手元に鍵材料が戻ってくる

#### escrow の決定と噛み合っている

control plane を正に据える判断（BitLocker と同じ三枚重ね）は、Pi に TPM が
無いことから来ていた。**NetBSD ではそれが好みではなく必然になる。** 他の
platform では control plane は「便利で中央管理できる」だが、NetBSD では
「これ以外に健全な消去が無い」。

逆に言えば、NetBSD にパスフレーズの非常口を足すことは、他の platform より
高くつく。足した瞬間、手元に消すべき物が生まれるため。**非常口を足すかどうかは
platform ごとに変えてよい**、という形になりそうだが、これは実装して確かめて
から決める。

#### まだ確かめていないこと

`shell_cmd` が回復環境と実機の両方で期待どおり動くか、実機で見ていない。
`cgdconfig` が鍵を stdout からどう読むか（長さ、符号化、改行の扱い）も
man からは読み取れていない。実装はそこを確かめてから。
