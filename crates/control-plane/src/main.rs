//! 鍵を預かり、消すときに失効させる。
//!
//! 回復環境は台ごとに違う鍵で root を暗号化し、消去はその鍵を壊すことで
//! 成り立つ。鍵の置き場がそのまま「本当に消えたか」を決めるので、ここが
//! その置き場になる。詳しくは回復環境側の `escrow` を見ること。
//!
//! **NetBSD ではこれが必須。** cgd は keyslot を持たないので鍵を手元に
//! 置けず、消去はここでの失効そのものになる。
//!
//! ```text
//! POST /v1/escrow            預かる
//! GET  /v1/key/<device_id>   渡す
//! POST /v1/revoke/<id>       失効させる
//! ```
//!
//! **TLS はここで終端しない。** 前段に任せる。回復環境は https でなければ
//! 鍵をやりとりしないので、前段が無ければ繋がらないだけで済む。
//!
//! **台ごとに token を分けていない。一台分が漏れれば全台の鍵が取れる。**
//! 分けるには台の登録という別の仕組みが要る。運用に載せる前に直すこと。
